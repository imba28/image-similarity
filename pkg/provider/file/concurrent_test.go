...
func TestUnitConcurrentImageProvider_Images(t *testing.T) {
	provider := ConcurrentImageProvider{Dir: "../../../test_sets"}

	files, err := provider.Images()
	if err != nil {
		t.Error("ConcurrentImageProvider should not return error if directory exists")
	}
	if len(files) != 7 {
...
func TestUnitConcurrentImageProvider_Get(t *testing.T) {
	provider := ConcurrentImageProvider{Dir: "../../../test_sets"}
...
func TestUnitConcurrentImageProvider_Persist(t *testing.T) {
	provider := ConcurrentImageProvider{Dir: "../../../test_sets"}
